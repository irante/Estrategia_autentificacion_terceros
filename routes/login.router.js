  ...
  if (user.password !== user.password2) {                 // si los password son diferentes
    return res.render('signup', {
      error: 'Las contraseñas no coinciden'
        ...
        // crear al usuario                               // si el usuario no existe, crear el usuario
  try {
    const newUser = await userManager.create({
      ...user,
      password: hashPassword(user.password)               // Creamos el objeto newUser con los datos del objeto user + propiedad password con el password hasheado (...spread)
    })
...
const login = async (req, res) => {
  ...
  try {
    ...
    if(!isValidPassword(password, _password)) {                           // usamos utils/password.utils.js (pass no encriptado, pass encriptado)
